...
class Dataset4PRVR(data.Dataset):
    ...
    def __init__(self, cap_file, visual_feat, text_feat_path, cfg, video2frames=None):
        ...
        if self.config['caption_rate'] > 0:
            if cfg['dataset_name'] =='charades':
                self.query_feat_file_caption_gpt = '/share/home/chenyaofo/project/chenchuanshen/wfq/save/LastVers/charades_train_020_merged.hdf5'
        ...
        if cfg.get('global_caption', False):
            if cfg['dataset_name'] =='charades':
                self.query_feat_file_global_caption = '/share/home/chenyaofo/project/chenchuanshen/wfq/WorkSpace/ms-sl/dataset/charades_global_qwen32b.hdf5'
            elif cfg['dataset_name'] =='activitynet':
    ...
    def __getitem__(self, index):
        ...
        # frame_vecs = []
        i3d_mdoel_feat = self.video_feat_file[video_id]['i3d_feat'][...]
        
        add_clip = False
        if add_clip: #add clip表示两个特征进行融合
            clip_model_feat = self.video_feat_file[video_id]['clip_feat'][...]
            # print(i3d_mdoel_feat.shape, clip_model_feat.shape)
            min_len = min(len(clip_model_feat), len(i3d_mdoel_feat))
            frame_vecs = np.concatenate((i3d_mdoel_feat[:min_len], clip_model_feat[:min_len]), axis=-1)
        else:
            frame_vecs = i3d_mdoel_feat
        # for frame_id in frame_list:
        #     if add_clip: #add clip表示两个特征进行融合
        #         clip_model_feat_one = clip_model_feat[min(cnt, len(clip_model_feat)-1)] #防止越界问题发生
        #         i3d_mdoel_feat_one = self.visual_feat.read_one(frame_id)
        #         temp_feat = np.concat(i3d_mdoel_feat_one, clip_model_feat_one)
        #         cnt += 1
        #         frame_vecs.append(temp_feat)
        #     else:

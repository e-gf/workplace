...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        self.parser.add_argument("--hca_loss_type", type=str, default="margin")
        ...
        self.parser.add_argument('--dataset_name', type=str, default='charades')
        self.parser.add_argument('--text_feat_path', type=str, default='text_feat_path')
        self.parser.add_argument('--caption_train_txt', type=str, default='caption_train_txt')
        self.parser.add_argument('--caption_test_txt', type=str, default='caption_test_txt')
        self.parser.add_argument("--caption_rate", type=float, default=0.0)
        self.parser.add_argument("--local_hinge_weight", type=float, help="weight for local hinge loss")
        self.parser.add_argument("--local_margin", type=float, help="margin for local hinge loss")
        self.parser.add_argument("--global_caption", action='store_true', help="global caption，default to be False")
        self.parser.add_argument("--global_soft_pos_weight", type=float, help="weight for global soft pos loss")
        self.parser.add_argument("--soft_pos_margin", type=float, help="margin for global soft hinge pos loss")
        self.parser.add_argument("--query_or_caption", type=int, help="chosseing the data used in original loss. 0 query; 1 global; 2 local; 3 all")
        self.parser.add_argument("--plot_losses", nargs='+', default=["loss_overall"], help="the loss names to plot, separated by spaces")
        self.parser.add_argument("--window_size", type=int, help="window size for global soft hinge loss")
    ...
    def parse(self):
        if not self.initialized:
            self.initialize() # 进行初始化，传入一些参数
        opt = self.parser.parse_args() # 验证这些参数是不是规定的合法参数
        if opt.debug:

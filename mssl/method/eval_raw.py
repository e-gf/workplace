...
def compute_context_info(model, eval_dataset, opt):
    ...
    for idx, batch in tqdm(enumerate(context_dataloader), desc="Computing query2video scores",
        ...
        _frame_feat, _video_proposal_feat, _  = model.encode_context(clip_video_feat_, frame_video_feat_, frame_mask_)
...
def compute_query2ctx_info(model, eval_dataset, opt, ctx_info):
    ...
    for idx, batch in tqdm(enumerate(query_eval_loader), desc="Computing q embedding", total=len(query_eval_loader)):
        ...
        query_metas.extend(batch[-2])
        query_metas_raw.extend(batch[-1])
        ...
        best_clip_indices.append(key_clip_indices.cpu().numpy())
        
        del _clip_scale_scores, _frame_scale_scores, _score_sum
...
def eval_epoch(model, val_video_dataset, val_text_dataset, opt):
    ...
    if 0:
        import json
        with open('/share/home/chenyaofo/project/chenchuanshen/ms-sl/visual_result/ms-sl-index2startend_ranges.json', 'r') as f:
            index2startend_ranges = json.load(f)
        best_clip_indices_final = []
        for i in range(len(best_clip_indices)):
            if i in t2v_gt:  # 简化判断，字典的in操作直接检查键
                # 获取t2v_gt[i]中的索引
                best_clip = t2v_gt[i][0]
                # 获取对应的起始和结束位置
                start, end = index2startend_ranges[str(best_clip_indices[i][best_clip])]
                # 分别除以32并添加到结果列表
                best_clip_indices_final.append([start / 32, end / 32])
            else:
                raise ValueError(f"Key {i} not found in t2v_gt")  # 抛出有意义的错误
            
        indices = np.where(clip_gt_ranks > 100)[0]

        # # 根据索引提取目标列表中的内容
        out100_query_metas = [query_metas[i] for i in indices]
        out100_query_metas_raw = [query_metas_raw[i] for i in indices]
        out100_query_best_clip_s_e = [best_clip_indices_final[i] for i in indices]
        # 保存到TXT文件
        with open("/share/home/chenyaofo/project/chenchuanshen/ms-sl/visual_result/ms-sl_噪声可视化.txt", "w") as f:
            # 写入标题行
            f.write("indices  out100_query_metas  raw_out100_query_metas  out100_query_best_clip_s_e\n")
            # 逐行写入索引和对应的值，用制表符分隔
            for idx, query_meta_item, raw_query_meta_item, clip_s_e in zip(indices, out100_query_metas, out100_query_metas_raw, out100_query_best_clip_s_e):
                f.write(f"{idx}\t{query_meta_item}\t{raw_query_meta_item}\t{clip_s_e}\n")

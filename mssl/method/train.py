...
def train_epoch(model, train_loader, optimizer, opt, epoch_i, training=True):
    ...
    loss_meters = OrderedDict(clip_nce_loss=AverageMeter(), clip_trip_loss=AverageMeter(),
                            ...
                            local_hinge_loss=AverageMeter(),
                            global_soft_pos_loss=AverageMeter())
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader), desc="Training Iteration", total=num_training_examples):
        ...
        # 判断是否要转换成tensor，新增的参数要分辨清楚！
        for k in batch.keys():
            if k != 'text_labels' and k != 'global_caption_mask':
        ...
        timer_start = time.time()
        loss, loss_dict = model(batch['clip_video_features'],batch['frame_video_features'],
                                batch['videos_mask'],batch['text_feat'],batch['text_mask'],
                                batch['text_labels'],batch['global_caption_mask'],batch['local_start_end_tensor']
                                ,current_epoch=epoch_i) #调用model.py的81行
...
def train(model, train_dataset, val_video_dataset, val_text_dataset, opt):
    ...
    for epoch_i in trange(start_epoch, opt.n_epoch, desc="Epoch"):
        ...
        # else:
        #     # 加载检查点文件
        #     checkpoint = torch.load('/share/home/chenyaofo/project/chenchuanshen/wfq/WorkSpace/ms-sl/charades/results/charades-with-directappend-softloss_local-w0.4-m0.2_global-w0.3-m0.1-2025_06_24_00_39_19/model.ckpt')['model']
...
def start_training(opt):
    ...
    # text_feat_path = '/share/home/chenyaofo/project/chenchuanshen/ms-sl_gt-main/dataset/charades_i3d/TextData/roberta_charades_query_feat.hdf5'
    ...
    # text_feat_path = os.path.join(rootpath, "dataset", dataset_files, 'TextData', 'roberta_%s_query_feat.hdf5' % opt.dataset_name)
    print("正在初始化dataset")
    ...
    # Load visual features
    # visual_feat_path = os.path.join(opt.root_path, 'dataset', dataset_files, 'FeatureData', opt.visual_feature)
    # video2frames = read_dict(os.path.join(opt.root_path, 'dataset', dataset_files, 'FeatureData', opt.visual_feature, 'video2frames.txt'))

    train_dataset = Dataset4MS_SL(opt.caption_train_txt, None, text_feat_path, opt, video2frames=None)
    print("train_dataset初始化完成") # 用于训练
    ...
    val_video_dataset = VisDataSet4MS_SL(None, None, opt, video_ids=val_video_ids_list)
    print("dataset初始化完成") # 用于验证
    ...
    model_config = EDict(
        ...
        hca_loss_type=opt.hca_loss_type
    )
...
if __name__ == '__main__':
    ...
    if not debug:
        # plotter = LossPlotter(opt.results_dir, opt.plot_losses)
        # plotter.plot_losses()

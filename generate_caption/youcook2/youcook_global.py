    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        with tqdm(total=len(filtered_video_folders), desc="Processing videos") as pbar:
            for batch_idx in range(total_batches):
                ...
                # 提交当前批次任务
                futures = []
                for video_path in current_batch:
...
def process_single_video(video_path, frames_folder, examples, use_gpt=True, model=None, processor=None, device=None):
    ...
    try:
        ...
        frames_data = load_frames_from_folder(video_path)
        if use_gpt:
...
def load_frames_from_folder(video_path: str) -> List[Dict]:
    """从视频文件抽帧并预处理（全区间0.0-1.0）
    返回格式: [{
...
def load_examples(json_path: str) -> List[str]:
    """从Youcook2 JSON文件中加载示例，从database中提取所有sentence值"""
    with open(json_path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    
    # 从database中提取所有sentence值
    examples = []
...
def get_random_examples(examples: List[str], num_examples: int = 10) -> str:
    """随机选择指定数量的示例"""
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_gpt', type=lambda x: (str(x).lower() == 'true'), default=True, help='是否使用gpt（True/False）')
    parser.add_argument('--device', type=str, default='cuda:0', help='Qwen推理设备，仅在use_gpt=False时有效')
    parser.add_argument('--frames_folder', type=str, help='图片帧文件夹路径')
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        for line in file:
            # 每行格式为：组文件夹/video_id，如 "405/QFb95r3q3tQ"
            line = line.strip()
    ...
    if not examples:
        raise ValueError("No examples found in file_path")
    ...
    # 视频文件在raw_videos目录下的子目录中
    raw_videos_folder = frames_folder
    if not os.path.exists(raw_videos_folder):
        print(f"原始视频文件夹不存在: {raw_videos_folder}")
        exit(1)
    
    # 遍历所有子目录（training, validation, testing）
    for subset in ['training', 'validation', 'testing']:
        subset_path = os.path.join(raw_videos_folder, subset)
        if not os.path.exists(subset_path):
            continue
            
        for group_folder in os.listdir(subset_path):
            group_path = os.path.join(subset_path, group_folder)
            ...
            for fname in os.listdir(group_path):
                ...
                if fname.lower().endswith(('.mp4', '.avi', '.mov', '.mkv', '.wmv', '.flv', '.webm')):
                    ...
                    # 构建完整的video ID路径（组文件夹/video_id）
                    full_video_id = f"{group_folder}/{video_id}"
                    if full_video_id in ids and video_id not in finished_ids:
                        # 存储完整的文件路径
                        full_path = os.path.join(group_path, fname)
                        filtered_video_folders.append(full_path)

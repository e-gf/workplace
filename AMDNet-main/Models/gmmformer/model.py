...
class GMMFormer_Net(nn.Module):
    ...
    def forward(self, batch):
        ...
        vid_proposal_feat, gmm_masks, proposal_fea, video, mapped_clip_feat = self.encode_context(clip_video_feat) # [bs, 384], [bs, 32, 384], ours[bs,528,384]
       

        clip_scale_scores, clip_scale_scores_, clip_scores,mapped_raw_clip_scale_scores = self.get_pred_from_raw_query(
            query_feat, query_mask, query_labels, vid_proposal_feat, proposal_fea, return_query_feats=True,
            global_caption_mask=global_caption_mask, 
            mapped_clip_feat=mapped_clip_feat)
        ...
        return [rank_loss, clip_scale_scores, clip_scale_scores_, label_dict, video_query, gmm_masks, clip_scores,mapped_raw_clip_scale_scores,global_caption_mask, local_start_end_tensor]
    ...
    def encode_context(self, clip_video_feat):
        ...
        encoded_clip_feat, gmm_masks, proposal_fea, video  = self.encode_input(clip_video_feat, None, self.clip_input_proj, self.clip_encoder,
                                               self.clip_pos_embed)
        #TODO: 需要看看用encoded_clip_feat还是哪个，不知道哪个是32,384的
        vid_proposal_feat_map = self.encode_feat_map(encoded_clip_feat)

        return encoded_clip_feat, gmm_masks, proposal_fea, video, vid_proposal_feat_map
    ...
    def get_pred_from_raw_query(self, query_feat, query_mask, query_labels=None,
        ...
        if return_query_feats: 
            ...
            return clip_scale_scores, clip_scale_scores_, query_context_scores,mapped_raw_clip_scale_scores # training
        else:
